...
# ne pas definir de chemin absolu pour un projet git car personel.
...
# j'importe les données avec read_csv2 parce que c'est un csv avec des ; et que read_csv attend
# comme separateur des ,
df <- readr::read_csv2(
  "individu_reg.csv",
  col_select = c("region", "aemm", "aged", "anai", "catl", "cs1", "cs2", "cs3", "couple", 
                 "na38", "naf08", "pnai12", "sexe", "surf", "tp", "trans", "ur")
)
...
# api_token <- "trotskitueleski$1917" a supprimer en exo
...
MASS::polr(surf ~ cs1 + factor(ur), df3) #relancer R ou faire ligne par ligne
